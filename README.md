...
- [Конфігурація](#конфігурація)
  ...
  - [Purity (оцінка якості)](#purity-оцінка-якості)
...
- [Ітеративний тюнінг](#ітеративний-тюнінг-iterative_tuningpy)
  ...
  - [Алгоритм пошуку (evolution)](#алгоритм-пошуку-evolution)
...
└── metrics/
    ├── global_run_config.json
    ├── _cache_config.json
    ├── purity_report_latest.json
    └── centroid_cosine_matrix.npy
```
...
| `output_dir` | string | `""` | Шлях куди будуть розкладені зображення. Створюється автоматично. Всередині: `cluster_000/`, `cluster_001/`, ..., `viz/` (візуалізації) і `metrics/` (purity-артефакти). |
...
### Purity (оцінка якості)

Секція `purity:` не змінює саму кластеризацію; вона оцінює якість ізоляції кластерів після `HDBSCAN`.

| Параметр | Тип | Опис |
|---|---|---|
| `purity.enabled` | bool | Чи обчислювати purity-метрики і зберігати звіт. |
| `purity.fail_on_fail` | bool | Якщо `true` — `main.py` блокує розкладання файлів при `verdict=FAIL`. Якщо `false` — метрики лише показуються, рішення лишається за оператором. |
| `purity.max_centroid_cosine` | float | Поріг для міжкластерної подібності центрів (менше краще). |
| `purity.min_silhouette` | float | Поріг для глобальної роздільності (більше краще). |
| `purity.max_nn_cross_ratio` | float | Поріг для локального міжкластерного перетоку в kNN (менше краще). |
| `purity.k_neighbors` | int | Кількість сусідів для розрахунку `max_nn_cross_ratio`. |
| `purity.silhouette_sample_size` | int | Семплінг для silhouette, щоб уникати дорогого `O(N^2)` на повному наборі. |
| `purity.silhouette_random_state` | int | Seed для відтворюваного семплінгу silhouette. |

Важливо:
- При `fail_on_fail: false` пороги все одно беруть участь у `checks` та `verdict` звіту.
- Метрики (`max_centroid_cosine`, `silhouette`, `max_nn_cross_ratio`) рахуються в будь-якому разі, якщо `purity.enabled: true`.
...
- Ранжувати конфіги за композитним `score` без жорсткого відсікання по `PASS/FAIL`.
...
2. Формування кандидатів (тільки evolution).
...
### Алгоритм пошуку (evolution)

- `SEARCH_STRATEGY = "evolution"`:
  - працює популяціями (elite + tournament selection + crossover + mutation),
  - швидше знаходить сильні конфіги при обмеженому бюджеті,
  - краще масштабується на великі простори параметрів,
  - автоматично враховує `eval_scope` (`subset`/`full`) у runtime-логах та артефактах.
...
- `TWO_PHASE_ENABLED`
- `PHASE1_FRACTION`
- `SWITCH_PATIENCE_GENERATIONS`
- `SWITCH_MIN_EVALUATIONS`
- `SWITCH_MIN_REL_IMPROVEMENT`
- `SWITCH_REEVAL_ELITES_TOP_K`

Логіка:

1. **Старт**: еволюція запускається на `subset` (`PHASE1_FRACTION`), якщо `TWO_PHASE_ENABLED=True`.
2. **Адаптивний свіч**: при плато або повільному прогресі виконується `subset -> full`.
3. **Після свічу**: кращі еліти переоцінюються на full (`SWITCH_REEVAL_ELITES_TOP_K`) і стають seeds наступних поколінь.
...
Керування вагами:
- ваги задані константами `SCORE_WEIGHT_*` у `iterative_tuning.py`;
- збільшити `SCORE_WEIGHT_MAX_COS` — сильніше тиснути на розділення центрів;
...
- Артефакти run-root:
  ...
  - `_cache_config.json` (копія cache-конфігу, що відповідає використаному feature-кешу)
- Артефакти ітерацій: `<run_root>/<iteration_name>/...`
  - за замовчуванням ім'я ітерації компактне (`evo_phase2_0076`), керується `USE_COMPACT_ITERATION_NAMES`
  - `iteration_config.json` (лише змінені параметри відносно `config.yaml`: `changed_params`)
  - `metrics/iteration_metric_<iteration_name>.json`
  - `metrics/cosine_<iteration_name>.npy`
- Агрегований підсумок:
  - `<run_root>/tuning_summary_latest.json`
  - `<run_root>/tuning_results_all.csv`
    - містить всі ітерації (`section=all`) і TOP-блок внизу (`section=top`, `rank=1..20`)
  - `<run_root>/_top_visuals/top_XX_<iteration_name>/viz/...` (тільки візуалізації top-кандидатів)
- Кеш індексу:
...
- У підсумковому ранжуванні пріоритет мають full-evals; якщо їх ще немає, використовується fallback на всі успішні ітерації.
- Наприкінці — готовий рейтинг (`score`) і повні конфіги для відтворення будь-якої ітерації.
